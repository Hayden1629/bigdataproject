...
def _clean_display_name(raw_name: str, fallback_query: str) -> str:
    ...
    return f"{base.title()} ({brand})" if brand else base.title()
...
def _empty_state() -> None:
    render_table(queries.load_drug_summary().head(20), height=420)
...
def _render_default_view(bundle: dict, approval: dict, label: dict) -> None:
    ...
    with c1:
        metric_card("Total Cases", format_compact(k["cases"]))
    with c2:
        metric_card("Deaths", format_compact(k["deaths"]), f"{k['death_pct']:.1f}%")
    with c3:
        metric_card("Hospitalisations", format_compact(k["hospitalisations"]))
    with c4:
        metric_card("Life-threatening", format_compact(k["life_threatening"]))
    with c5:
        metric_card("Any Serious Outcome", format_compact(k["serious"]), f"{k['serious_pct']:.1f}%")

    if approval:
        render_section_intro("FDA Regulatory Snapshot")
        with st.container(border=True):
            st.write(f"Application number: **{approval.get('application_number', '-') or '-'}**")
            st.write(f"Sponsor: **{approval.get('sponsor', '-') or '-'}**")
            st.write(f"First approval date: **{approval.get('first_approval_date', '-') or '-'}**")
            st.write(f"Latest action date: **{approval.get('latest_action_date', '-') or '-'}**")
            st.write(f"Dosage form: **{approval.get('dosage_form', '-') or '-'}**")
            st.write(f"Route: **{approval.get('route', '-') or '-'}**")
            st.write(f"Marketing status: **{approval.get('marketing_status', '-') or '-'}**")
    ...
    render_section_intro("Recent Drug Records")
...
def render(filters: dict) -> None:
    render_section_intro("Drug explorer")
    q = st.text_input("Search a drug", placeholder="e.g., metformin", key="drug_query")
    render_helper_text("Try examples: metformin, ozempic, acetaminophen, adderall")
    if not q.strip():
        _empty_state()
        return
